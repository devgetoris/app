...
export function InlineSearchResults({
  ...
  if (results && results.length > 0) {
    console.log(
      "📋 Sample result in InlineSearchResults:",
  ...
  const filteredResults = results.filter(result => {
    ...
    if (emailFilter === "unlocked")
      return result.email && result.email !== "email_not_unlocked";
  ...
  const hasPeople = results.some(
    result =>
      result.recordType === "individual" ||
      (!result.recordType && (result.firstName || result.lastName))
  );
  const hasOrganizations = results.some(
    result =>
      result.recordType === "organization" ||
      (!result.recordType &&
        result.name &&
        !result.firstName &&
        !result.lastName)
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast.success(
        `Successfully imported ${selectedIds.length} ${resultType}`
      );
      setSelectedIds([]);
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex-1">
            <CardTitle>
              Search Results ({filteredResults.length} {resultType})
            </CardTitle>
            <CardDescription>
              Review and select the {resultType} you want to import to your
              leads.
            </CardDescription>
          </div>
                      ...
                      <>
                        ...
                        {result.apolloData?.alexa_ranking && (
                          <div className="text-xs flex items-center gap-1">
                            ...
                            <span>
                              #
                              {result.apolloData.alexa_ranking.toLocaleString()}
                            </span>
                          </div>
                        )}

                        {result.apolloData?.retail_location_count !==
                          undefined &&
                          result.apolloData.retail_location_count > 0 && (
                            <div className="text-xs flex items-center gap-1">
                        ...
                        {/* Keywords */}
                        {result.apolloData?.keywords &&
                          result.apolloData.keywords.length > 0 && (
                            <div className="pt-2">
