...
export async function POST(request: NextRequest) {
  try {
    ...
    if (apolloId) {
      ...
      console.log(
        `✅ Retrieved contact by ID. Email: ${contact?.email || "NO EMAIL"}`
      );
    } else if (email) {
      ...
      contact = response.person;
      console.log(
        `✅ Retrieved contact by email. Email: ${contact?.email || "NO EMAIL"}`
      );
    } else {
    ...
    console.log(
      `✨ Enrich complete. Returning contact with email: ${contact.email}`
    );
