...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      person_titles &&
      Array.isArray(person_titles) &&
      person_titles.length > 0
    ) {
    ...
    if (
      person_seniorities &&
      Array.isArray(person_seniorities) &&
      person_seniorities.length > 0
    ) {
    ...
    if (
      person_locations &&
      Array.isArray(person_locations) &&
      person_locations.length > 0
    ) {
    ...
    if (
      contact_email_status &&
      Array.isArray(contact_email_status) &&
      contact_email_status.length > 0
    ) {
    ...
    console.log(
      "🔍 Apollo People Search - Using comprehensive search strategy"
    );
    ...
    const isVerySpecific =
      ...
      (searchParams.person_titles?.length === 1 &&
        searchParams.person_locations?.length === 1);

    if (isVerySpecific) {
      console.log(
        "📈 Apollo People Search - Very specific criteria detected, adding broader search parameters"
      );
    ...
    if (allContacts.length === 0) {
      console.log(
        "🔄 Apollo People Search - No results found, trying broader search..."
      );
      ...
      if (searchParams.organization_locations?.length > 0) {
        broaderParams.organization_locations =
          searchParams.organization_locations;
      }

      console.log(
        "🔍 Apollo People Search - Broader search params:",
      ...
      try {
        ...
        console.log(
          `📈 Apollo People Search - Broader search found ${allContacts.length} contacts`
        );
      } catch (error) {
        console.log(
          "⚠️ Apollo People Search - Broader search also failed:",
          error
        );
      }
    }
    ...
    console.log(
      `📦 Bulk Enrichment - Processing ${allContacts.length} contacts in ${contactBatches.length} batch(es)`
    );
    ...
    for (let batchIdx = 0; batchIdx < contactBatches.length; batchIdx++) {
      ...
      for (const contact of batch) {
        try {
          ...
          console.log(
            `⚠️ Failed to enrich ${contact.first_name} ${contact.last_name} (${contact.id}), using search data: ${error instanceof Error ? error.message : "Unknown error"}`
          );
      ...
      console.log(
        `✅ Batch enrichment complete. Enriched ${enrichedBatch.length} of ${batch.length} contacts`
      );

      // Process each enriched contact
      for (
        let contactIdx = 0;
        contactIdx < enrichedBatch.length;
        contactIdx++
      ) {
        ...
        console.log(
          `   Initial email from search: ${originalContact.email || "NO EMAIL"}`
        );
        console.log(
          `   ✅ Enrichment successful. Email: ${enrichedContact.email || "NO EMAIL"}`
        );
        console.log(
          `   ℹ️ Email status: ${enrichedContact.email || "NO EMAIL"}`
        );
        ...
        const leadData = {
          ...
          employmentHistory: enrichedContact.employment_history?.map(
            (job: any) => ({
              title: job.title || "",
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Failed to search people",
      },
      { status: 500 }
    );
  }
