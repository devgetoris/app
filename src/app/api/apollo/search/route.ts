  ...
  try {
    ...
    for (let batchIdx = 0; batchIdx < contactBatches.length; batchIdx++) {
      ...
      for (
        ...
        if (!existingLead) {
          console.log(
            `   💾 Saving new lead with email: ${enrichedContact.email}`
          );
          const [newLead] = await db
            ...
            .values({
              ...
              companySize: enrichedContact.organization?.estimated_num_employees
                ...
                : null,
              companyRevenue:
                enrichedContact.organization?.annual_revenue_printed,
              ...
              companyFunding:
                enrichedContact.organization?.publicly_traded_symbol,
              companyTechnologies:
                enrichedContact.organization?.technology_names,
    ...
    if (savedLeads.length > 0) {
      console.log(
        "📋 Sample lead data being returned:",
        JSON.stringify(savedLeads[0], null, 2)
      );
    }
