...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const result of selectedResults) {
      ...
      if (!existingLead) {
        const recordType =
          searchType === "organizations" ? "organization" : "individual";
        const displayName =
        ...
        console.log(
          `💾 Importing ${recordType}: ${displayName} (ID: ${apolloId})`
        );
        ...
        const [newLead] = await db
          ...
          .values({
            ...
            companyName:
              searchType === "organizations" ? result.name : result.companyName,
            companyDomain:
              searchType === "organizations"
                ? result.domain
            ...
            companyLocation:
              searchType === "organizations"
                ? `${result.location?.city || ""}, ${result.location?.state || ""}, ${result.location?.country || ""}`.replace(
                    /^,\s*|,\s*$/g,
                    ""
                  )
            ...
            companyFunding:
              searchType === "organizations"
                ? result.publiclyTraded
                : result.companyFunding,
            companyTechnologies:
              searchType === "organizations"
                ? result.technologies
                : result.companyTechnologies,
